...
def get_session_token_info(profile_name: str, config_file_path: Optional[str] = None) -> Optional[dict]:
    """
...
def setup_session_token(project_name: str, stage: str, region: str) -> str:
    ...
    if check_session_token_validity(target_profile):
        ...
        if token_info:
            ...
            display_success(f"✓ Using existing valid session token for profile '{target_profile}' (age: {age_minutes:.1f} minutes)")
