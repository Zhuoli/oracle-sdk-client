"""
SSH Config generation utilities for integration with main demo.
"""
...
def generate_ssh_config_entries(
    ...
    if oke_instances:
        console.print(f"[bold cyan]Generating SSH config for {len(oke_instances)} OKE instances[/bold cyan]")
        cluster_counts = {}
        ...
        for instance in oke_instances:
            ...
            if not bastion:
                console.print(f"[yellow]No bastion found for OKE instance {instance.instance_id}[/yellow]")
                continue
    ...
    if odo_instances:
        console.print(f"[bold cyan]Generating SSH config for {len(odo_instances)} ODO instances[/bold cyan]")
        
        for i, instance in enumerate(odo_instances, 1):
            # Find matching bastion
            bastion = client.find_bastion_for_subnet(bastions, instance.subnet_id)
            if not bastion:
                console.print(f"[yellow]No bastion found for ODO instance {instance.instance_id}[/yellow]")
                continue
...
def write_ssh_config_file(
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"# Generated by OCI Python Client Demo\n")
