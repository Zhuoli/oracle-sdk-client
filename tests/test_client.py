...
from src.oci_client.models import (
    InstanceInfo, BastionInfo, LifecycleState, BastionType,
    RegionInfo, AuthType
)
...
class TestOCIClient:
    ...
    def test_get_internal_domain(self, mock_requests, mock_client):
        ...
        mock_response.json.return_value = {
            "internal_realm_domain": "internal.oraclecloud.com"
        }
        mock_requests.return_value = mock_response
        
        with patch.object(mock_client, 'get_region_info') as mock_region:
            mock_region.return_value = RegionInfo(
                name="us-ashburn-1",
                key="iad"
            )
        ...
        oke_instance = InstanceInfo(
            ...
            metadata={
                ...
                "oke-initial-node-labels": {
                    "tot.oraclecloud.com/node-pool-name": "pool1"
                }
            }
        )
