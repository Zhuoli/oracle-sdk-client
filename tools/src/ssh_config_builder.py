...
from oci_client.models import BastionType, LifecycleState
...
class SSHConfigBuilder:
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            self.client = OCIClient(region=self.region, profile_name=self.profile_name)
    ...
    def build_oke_config(
        self, host_prefix: str = "today", cluster_name: Optional[str] = None
    ) -> List[Dict[str, str]]:
        ...
        oke_instances = self.client.list_oke_instances(
            compartment_id=self.compartment_id, cluster_name=cluster_name
        )
        ...
        bastions = self.client.list_bastions(
            compartment_id=self.compartment_id, bastion_type=BastionType.INTERNAL
        )
        ...
        for instance in oke_instances:
            ...
            if not bastion:
                console.print(
                    f"[yellow]No bastion found for instance {instance.instance_id}[/yellow]"
                )
                continue
    ...
    def build_odo_config(self, host_prefix: str = "today") -> List[Dict[str, str]]:
        """Build SSH config entries for ODO instances."""
        console.print("\n[bold blue]Building ODO instance SSH config...[/bold blue]")

        # Get ODO instances
        odo_instances = self.client.list_odo_instances(compartment_id=self.compartment_id)
    ...
    def write_config_file(
        self, config_entries: List[Dict[str, str]], output_file: str, update_existing: bool = True
    ) -> None:
...
Examples:
  ...
  python ssh_config_builder.py ocid1.compartment.oc1..xxxxx --type oke --cluster-name my-cluster
    ...
    parser.add_argument("compartment_id", help="OCI compartment OCID")
    ...
    parser.add_argument(
        "--region", default="us-phoenix-1", help="OCI region (default: us-phoenix-1)"
    )

    parser.add_argument("--realm", default="oc1", help="OCI realm (default: oc1)")

    parser.add_argument(
        "--host-prefix", default="today", help="Prefix for host names (default: today)"
    )

    parser.add_argument("--cluster-name", help="Specific OKE cluster name to filter")
    ...
    parser.add_argument(
        "--profile", help="OCI profile name (default: ssh_builder for OKE, ssh_builder_odo for ODO)"
    )
    ...
    if args.type in ["oke", "both"]:
        oke_configs = builder.build_oke_config(
            host_prefix=args.host_prefix, cluster_name=args.cluster_name
        )
        all_configs.extend(oke_configs)

    if args.type in ["odo", "both"]:
        odo_configs = builder.build_odo_config(host_prefix=args.host_prefix)
