...
class SSHConfigManager:
    ...
    def parse_config_file(file_path: str) -> Dict[str, SSHConfigEntry]:
        ...
        if current_host and current_hostname and current_proxy:
            entries[current_host] = SSHConfigEntry(
                host=current_host, hostname=current_hostname, proxy_command=current_proxy
            )
    ...
    def write_config_file(
        entries: List[SSHConfigEntry], file_path: str, update_existing: bool = True
    ) -> None:
    ...
    @staticmethod
    def update_or_add_entry(file_path: str, host: str, hostname: str, proxy_command: str) -> None:
        """Update existing entry or add new one."""
        entries = SSHConfigManager.parse_config_file(file_path)

        entries[host] = SSHConfigEntry(host=host, hostname=hostname, proxy_command=proxy_command)

        SSHConfigManager.write_config_file(list(entries.values()), file_path, update_existing=False)
