...
from ..models import CompartmentInfo, LifecycleState, RegionInfo
...
class IdentityService:
    ...
    def list_regions(self) -> List[RegionInfo]:
        ...
        try:
            ...
            return [
                RegionInfo(
                    name=region.name, key=region.key, realm_key=getattr(region, "realm_key", None)
                )
    ...
    def list_availability_domains(self, compartment_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"name": ad.name, "id": ad.id, "compartment_id": ad.compartment_id} for ad in ads
            ]
        except Exception as e:
                ...
                for comp in response.data:
                    compartments.append(
                        CompartmentInfo(
                            ...
                            time_created=(
                                comp.time_created.isoformat() if comp.time_created else None
                            ),
                        )
                    )
            ...
            return [
                {
                    ...
                    "time_created": (
                        policy.time_created.isoformat() if policy.time_created else None
                    ),
